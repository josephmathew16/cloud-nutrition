...
def main():
    ...
    df[["Protein_to_Carbs_ratio","Carbs_to_Fat_ratio"]] = \
        df[["Protein_to_Carbs_ratio","Carbs_to_Fat_ratio"]].fillna(0)

    # 5) Required summaries
    avg_by_diet = df.groupby("Diet_type")[["Protein(g)","Carbs(g)","Fat(g)"]].mean().reset_index()
    avg_by_cuisine = df.groupby("Cuisine_type")[["Protein(g)","Carbs(g)","Fat(g)"]].mean().reset_index()
    top5_per_diet = (
    ...
    most_common = (
        df.groupby(["Diet_type","Cuisine_type"]).size().reset_index(name="count")
          .sort_values(["Diet_type","count"], ascending=[True,False])
          .groupby("Diet_type").head(1).reset_index(drop=True)
    )
    highest_protein_diet = avg_by_diet.loc[avg_by_diet["Protein(g)"].idxmax(), "Diet_type"]
    ...
    write_csv(df[["Diet_type","Recipe_name","Cuisine_type","Protein(g)","Carbs(g)","Fat(g)",
                  "Protein_to_Carbs_ratio","Carbs_to_Fat_ratio"]], "macro_ratios.csv")

    # 7) Visuals (3 required)
    sns.set_theme()

    # A) Grouped bar: avg Protein/Carbs/Fat by Diet_type
    m = avg_by_diet.melt(id_vars="Diet_type",
                         value_vars=["Protein(g)","Carbs(g)","Fat(g)"],
                         var_name="Macro", value_name="grams")
    plt.figure(figsize=(11,6))
    sns.barplot(data=m, x="Diet_type", y="grams", hue="Macro")
    plt.title("Average Macros by Diet Type"); plt.xlabel("Diet Type"); plt.ylabel("grams")
    plt.tight_layout(); fp = OUT_DIR / "avg_macros_by_diet.png"; plt.savefig(fp, dpi=150); plt.close(); log(f"PLOT {fp}")

    # B) Heatmap: mean Protein by Diet × Cuisine
    pivot = df.pivot_table(index="Diet_type", columns="Cuisine_type", values="Protein(g)", aggfunc="mean", fill_value=0)
    plt.figure(figsize=(12,7))
    sns.heatmap(pivot, cmap="viridis")
    plt.title("Mean Protein (g) by Diet × Cuisine"); plt.tight_layout()
    fp = OUT_DIR / "diet_cuisine_heatmap.png"; plt.savefig(fp, dpi=150); plt.close(); log(f"PLOT {fp}")

    # C) Stripplot: Top-5 protein items per diet, across cuisines
    plt.figure(figsize=(12,6))
    sns.stripplot(data=top5_per_diet, x="Cuisine_type", y="Protein(g)", hue="Diet_type", dodge=True)
    plt.xticks(rotation=45); plt.title("Top-5 Protein-Rich Recipes per Diet (by Cuisine)")
    plt.tight_layout(); fp = OUT_DIR / "top5_per_diet_by_cuisine.png"; plt.savefig(fp, dpi=150); plt.close(); log(f"PLOT {fp}")
